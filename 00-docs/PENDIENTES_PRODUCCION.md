...
| ~~C5~~ | ✅ **Respaldo de leads ACTIVO (2026-07-05)** | BD Neon `neon-green-door` conectada al proyecto (integración marketplace, plan Free). Probado end-to-end en producción: lead de prueba → HTTP 201 → fila #1 en `leads_backup` con `hubspot_ok=false, email_ok=false` (correcto: aún sin credenciales C1). Recuperación: `SELECT * FROM leads_backup WHERE hubspot_ok IS NOT TRUE` |
